...
@pytest.fixture
def mini_index(tmp_path: Path):
    """Мини-индекс из двух чанков для изолированных тестов."""
    chunks = [
        {
            "chunk_id": "2_0",
            "doc_id": "2",
            "name": "Inception — начало",
            "text": "Продукт: фильм. Проблема: сны внутри снов. Дом Кобб крадёт идеи из подсознания.",
            "metadata": {"year": 2010, "director": "Nolan"},
        },
        {
            "chunk_id": "1_0",
            "doc_id": "1",
            ...
            "metadata": {"year": 2000, "director": "Unknown"},
        },
    ]
...
def test_search_returns_k_results(mini_index):
    ...
    results = r.search("сны подсознание", k=2)
    assert len(results) == 2

def test_search_results_have_doc_id_and_score(mini_index):
    r = Retriever(**mini_index)
    results = r.search("сны", k=TOP_K)
    assert results
    for hit in results:
...
def test_search_prefers_relevant_doc(mini_index):
    r = Retriever(**mini_index)
    results = r.search("сны Нолан", k=1)
...
def test_ask_sources_contain_doc_id(mini_index):
    result = ask("сны подсознание", retriever=Retriever(**mini_index))
